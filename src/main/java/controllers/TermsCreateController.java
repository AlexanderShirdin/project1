...
@WebServlet(name = "TermsCreateController", urlPatterns = "/term_creating")
public class TermsCreateController extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        DBManager manager = new DBManager();
        manager.getAllActiveDisciplines();
        List<Discipline> disciplines = manager.getAllActiveDisciplines();

        req.setAttribute("disciplines", disciplines);
        req.getRequestDispatcher("JSP/term_creating.jsp").forward(req, resp);
    }
